			...
			return Response.json(
				{
					success: false,
					message:
						"Denne kode er ikke genereret til dig.",
				},
				{ status: 401 }
			);
...
export async function PUT(req: Request) {
	const { code, username, uuid, verify } =
		await req.json();
	...
	if (
		parsed.forAccount !== username &&
		verify == "Minecraft"
	)
		return Response.json({
			success: false,
			message:
				"Koden er genereret for en anden konto.",
		});
